...
mod common;
use common::{AllocationTracker, AllocationStats};
...
fn test_allocation_pattern_basic_operations() {
    ...
    for i in 0..10 {
        ...
        println!("Insert #{}: allocations: +{}, current memory: {}",
            i,
    ...
    println!("Total read allocations: {}, memory delta: {}",
        read_end_stats.allocations - read_start_stats.allocations,
        AllocationStats::format_bytes(
            read_end_stats.current_bytes.saturating_sub(read_start_stats.current_bytes)
        )
    );
    ...
    println!("Total delete allocations: {}, final memory: {}",
    ...
    println!("Peak memory: {}", AllocationStats::format_bytes(final_stats.peak_bytes));
    println!("Current memory: {}", AllocationStats::format_bytes(final_stats.current_bytes));
...
fn test_transaction_rollback_allocations() {
    ...
    println!("Successful transaction - allocations: {}, memory: {}",
    ...
    println!("Rolled back transaction - allocations: {}, memory delta: {}",
        rollback_end.allocations - rollback_start.allocations,
        AllocationStats::format_bytes(
            rollback_end.current_bytes.saturating_sub(rollback_start.current_bytes)
        )
    );
    ...
    println!("Memory difference after rollback: {} bytes", memory_diff.abs());
}
...
fn test_concurrent_allocation_patterns() {
        ...
        let handle = thread::spawn(move || {
            for i in 0..25 {
                ...
                if i % 10 == 0 {
                    ...
                    println!("Thread {} at iteration {}: current memory: {}",
                        thread_id, i,
    ...
    println!("Total allocations: {}", end_stats.allocations - start_stats.allocations);
    println!("Peak memory: {}", AllocationStats::format_bytes(end_stats.peak_bytes));
    println!("Current memory: {}", AllocationStats::format_bytes(end_stats.current_bytes));
}

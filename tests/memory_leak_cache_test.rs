...
fn test_with_cache_size(cache_size: Option<usize>, test_name: &str) {
    ...
    if let Some(size) = cache_size {
        ...
        println!("Cache size set to: {} bytes ({:.2} MB)", size, size as f64 / 1024.0 / 1024.0);
    } else {
    ...
    if let Some(stats) = MemoryTracker::get_current_memory() {
        println!("After deletions - Physical: {}, Virtual: {}",
            AllocationStats::format_bytes(stats.physical),
            AllocationStats::format_bytes(stats.virtual_mem)
        );
    }
    ...
    if let Some(stats) = MemoryTracker::get_current_memory() {
        println!("After drop - Physical: {}, Virtual: {}",
            AllocationStats::format_bytes(stats.physical),
            AllocationStats::format_bytes(stats.virtual_mem)
        );
    }
...
fn test_compaction_behavior() {
    ...
    for cycle in 0..5 {
        ...
        if let Some(stats) = MemoryTracker::get_current_memory() {
            println!("After partial delete - Physical: {}", AllocationStats::format_bytes(stats.physical));
        }
    }

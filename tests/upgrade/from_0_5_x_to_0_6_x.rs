...
fn try_to_open_legacy_database_without_upgrade_feature() {
    ...
    let db_error: Result<Database<'_>, db_type::Error> =
        Builder::new().open(&models, &database_path);
...
#[test]
#[cfg(feature = "upgrade_0_5_x")]
fn try_to_open_legacy_database_with_upgrade_feature_with_other_model() {
    use std::path::PathBuf;
    #[cfg(any(target_os = "android", target_os = "ios"))]
    let database_path = { dinghy_test::test_project_path().join("tests/data/db_0_5_x") };
